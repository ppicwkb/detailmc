    ...
    populateDatalist('poList', createOptions('po'));
        ...
        function applyFilters() {
            ...
            filteredData = allData.filter(item => {
                ...
                return globalMatch &&
                    (filters.produkId === '' || (item.produkId || '').toLowerCase() === filters.produkId) &&
                    (filters.packing === '' || (item.packing || '').toLowerCase() === filters.packing) &&
                    (filters.brand === '' || (item.brand || '').toLowerCase() === filters.brand) &&
                    (filters.po === '' || (item.po || '').toLowerCase() === filters.po) &&
        ...
        function updateStats() {
            ...
            document.getElementById('totalProducts').textContent = formatNumber(filteredData.length);
        ...
        function updateDashboard() {
            ...
            const stats = { WKB: { saldo: 0, kg: 0 }, GIM: { saldo: 0, kg: 0 }, CONT: { saldo: 0, kg: 0 } };
            ...
            const total = { 
                saldo: stats.WKB.saldo + stats.GIM.saldo + stats.CONT.saldo, 
                kg: stats.WKB.kg + stats.GIM.kg + stats.CONT.kg
            };
            ...
            setVal('contSaldo', stats.CONT.saldo); setVal('contKg', stats.CONT.kg, 2);
